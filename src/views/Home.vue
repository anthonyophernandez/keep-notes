<template>
  <div class="relative w-full h-full">
    ...
    <main class="fixed inset-0 px-2 mt-16 overflow-y-auto" :class="(isMenuDisplayed) ? 'ml-20 sm:ml-64 sm:z-40' : 'ml-20'">
      <div class="relative flex flex-col w-full max-w-xl h-auto mx-auto mt-2 mb-8 border border-gray-700 rounded-lg">
        ...
        <div class="w-full h-full my-2 pl-4 pr-10" @click="isVisible = true">
          <textarea rows="2" v-if="isVisible" class="resize-y h-auto w-full mb-2 bg-transparent text-white text-lg font-bold placeholder-gray-500" placeholder="Title"></textarea>
          <textarea :rows="(isVisible) ? '3' : '1'" class="w-full bg-transparent text-white placeholder-gray-500" :class="(isVisible) ? 'h-auto resize-y' : 'h-6 resize-none'" placeholder="Take a note..."></textarea>
        </div>
